# Use singleton class Beehive::Preferences::Store.instance to access
#
...
module Beehive::Preferences
  class StoreInstance
    ...
    def exist?(key)
      @cache.exist?(key) ||
        should_persist? && Beehive::Preference.where(key: key).exists?
    end
    ...
    def get(key)
      ...
      if should_persist?
        ...
        val = if preference = Beehive::Preference.find_by(key: key)
          # it does exist
                preference.value
              else
    ...
    def persist(cache_key, value)
      ...
      preference = Beehive::Preference.where(key: cache_key).first_or_initialize
    ...
    def destroy(cache_key)
      ...
      preference = Beehive::Preference.find_by(key: cache_key)
      preference.destroy if preference
    end

    def should_persist?
      @persistence && Beehive::Preference.table_exists?
